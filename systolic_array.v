...
#( 
  ...
  // First declare the wires between MAC units
  ...
  // connect external ifmap to the first row and first column of the array
  generate
    for (y = 0; y < ARRAY_HEIGHT; y = y + 1)
      assign ifmap_w[0][y] = ifmap_in[y];
  endgenerate

  // connect external ofmap inputs to the first row of the array
  generate
    for (x = 0; x < ARRAY_WIDTH; x = x + 1)
  ...
  generate
    for (x = 0; x < ARRAY_WIDTH; x = x + 1)
      assign ofmap_out[x] = ofmap_w[x][ARRAY_HEIGHT];
  endgenerate
  // Your code ends here
endmodule
